...
async function main() {
  ...
  while (page <= totalPages) {
    ...
    const res = await fetch(url.toString(), { headers: { 'User-Agent': 'kdashboard lastfm-dryrun' } });
    if (!res.ok) {
    ...
    console.log(`Page ${page}/${totalPages} - raw returned: ${raw.length} (total attr.total=${total})`);
  ...
  const dated = all.filter(t => t.date && t.date.uts).map(t => ({
  ...
  console.log(`Number of titles with duplicates (count>1): ${duplicates.length}`);
